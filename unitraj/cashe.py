...
class SMARTDataset(BaseDataset):
        ...
        with h5py.File(hdf5_path, "w") as f:
            for cnt, file_name in enumerate(data_list):
                ...
                try:
                    ...
                    print("1111 Warning: {} in {}".format(e, file_name))
                    output = None
                try:
                    output = self.process(output)
                except Exception as e:
                    print("2222 Warning: {} in {}".format(e, file_name))
                    output = None
                try:
                    output = self.postprocess(output)
                except Exception as e:
                    print("3333 Warning: {} in {}".format(e, file_name))
                    output = None
    ...
    def preprocess(self, scenario):
        ...
        for k, v in map_feat.items():
            ...
            # ----------------------debug1------------------
            cur_info["polygon_type"] = polygon_type[polyline_type_]
    ...
    def smart_convert(self, output):
        ...
        for i in output:
            ...
            obj_his_heading_encoding = i["obj_trajs"][..., 20:22]
            ...
            velo_all = np.concatenate(
                [i["obj_trajs"][..., 22:24], i["obj_trajs_future_state"][..., 3:5]],
                axis=1,
            )
            ...
            # ——— 在这里把所有 numpy.ndarray 转成 torch.Tensor ———
            # for key, val in list(d.items()):
            #     if isinstance(val, NodeStorage):
            #         # NodeStorage 是一个 dict-like
            #         for attr, arr in list(val.items()):
            #             val[attr] = _recursive_to_tensor(arr)
            #     else:
            #         d[key] = _recursive_to_tensor(val)

            # ——到这里，d 里已不会再有任何 numpy.ndarray——
