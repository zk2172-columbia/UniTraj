...
def main(cfg):
    ...
    for lane in map_polylines:
        if np.allclose(lane, 0):  # padding lane
            continue
        ax.plot(lane[:, 0], lane[:, 1], color='grey', linewidth=0.5, alpha=0.3)
    ...
    for idx in tqdm(idxs):
        ...
        # 计算最优轨迹 (minADE)
        ade_all = np.linalg.norm(preds_local - gt_future_local[None,:,:], axis=-1).mean(-1)  # (num_modes,)
    ...
    print("Visualization saved to scene_prediction_multimodal_beautified.png")
