...
class BaseDataset(Dataset):
    ...
    def process_data_chunk(self, worker_index):
        ...
        try:
            ...
            print(f"❌ Failed to load data chunk {worker_index}: {e}")
            return {}
        ...
        with h5py.File(hdf5_path, 'w') as f:
            for cnt, file_name in enumerate(data_list):
                ...
                try:
                    ...
                    print(f"❌ read_scenario failed for {file_name}: {e}")
                    continue
                ...
                try:
                    ...
                    print(f"⚠️ Exception in processing pipeline for {file_name}: {e}")
                    import traceback
                    traceback.print_exc()
                    output = None

                if output is None:
                    print(f"⚠️ Output is None, skipping {file_name}")
                    continue
                ...
                for i, record in enumerate(output):
                    ...
                    try:
                        ...
                        print(f"⚠️ kalman_difficulty missing or invalid in {file_name}: {e}")
                        kalman_difficulty = np.zeros(len(output))
    ...
    def get_map_data(self, center_objects, map_infos):
        ...
        if len(polyline_list) == 0: return (
            np.zeros((num_agents, 0, max_points_per_lane, 7)),
            np.zeros((num_agents, 0, max_points_per_lane)),
            np.zeros((num_agents, 0, 3))
        )
        ...
        map_types_int = map_types.astype(int)
        map_types = np.eye(map_types_int.max() + 1)[map_types_int]
