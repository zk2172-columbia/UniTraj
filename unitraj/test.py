...
class SMARTDataset(BaseDataset):
    ...
    def preprocess(self, scenario):
        ...
        for k, v in map_feat.items():
            ...
            # 补充默认 polygon_type（谨慎兜底）
            if polyline_type_ in [18]:  # crosswalk
                cur_info["polygon_type"] = polygon_type["PEDESTRIAN"]
            elif polyline_type_ in [17, 19]:  # stop sign / speed bump
    ...
    def smart_convert(self, output):
        ...
        for i in output:
            ...
            print("hist shape:", i["obj_trajs"][..., 25:27].shape,
                  "fut shape:", i["obj_trajs_future_state"][..., 3:5].shape,
                  "scenario:", i["scenario_id"])
            print("obj_trajs shape:", i["obj_trajs"].shape)
            print("obj_trajs[..., 25:27] shape:", i["obj_trajs"][..., 25:27].shape)
